...
export function PriceDisplay({ price, size = 'base', originalPrice }) {
  ...
  const sizeClass = size === 'xl' ? 'text-price-xl' : size === 'lg' ? 'text-price-lg' : 'text-price'
...
export function StarRating({ rating, count }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <div className="flex">
        {stars.map((type, i) => (
          <span key={i} className={type === 'empty' ? 'text-text-muted' : 'text-[#FF9F00]'}>★</span>
        ))}
      </div>
      <span className="text-xs text-trust">{rating}</span>
      {count && <span className="text-xs text-text-muted">({count.toLocaleString('en-IN')})</span>}
    </div>
  )
}
...
export function SkeletonCard() {
  return (
    <div className="bg-surface rounded border border-border-default p-md animate-pulse" aria-busy="true">
...
export function QuantitySelector({ value, onChange, min = 1, max = MAX_CART_QUANTITY }) {
  return (
    <div className="flex items-center border border-border-default rounded overflow-hidden">
      ...
      <span className="px-md py-xs text-sm font-medium min-w-[2rem] text-center">{value}</span>
      <button
...
export function ProgressBar({ value, max, color = 'buy-primary' }) {
  ...
  return (
    <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
      <div
        className={`h-full bg-${color} rounded-full transition-all duration-slow`}
        style={{ width: `${pct}%` }}
      />
    </div>
  )

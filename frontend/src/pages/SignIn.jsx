...
export default function SignIn() {
    ...
    const handleGoogle = async () => {
        try {
            ...
            const msg = err.message || (err.code ? String(err.code) : 'Google sign-in failed.');
            toast.error(msg);
    ...
    return (
        <div className="min-h-screen bg-page-bg flex items-center justify-center p-lg">
            <div className="bg-surface rounded-lg shadow-sm border border-border-default p-xl w-full max-w-md">
                ...
                {!firebaseConfigured && (
                    <div className="mb-lg p-md rounded bg-amber-500/10 border border-amber-500/30 text-amber-700 dark:text-amber-400 text-sm">
                        Sign-in is disabled: add <code className="bg-black/10 px-1 rounded">VITE_FIREBASE_API_KEY</code> and <code className="bg-black/10 px-1 rounded">VITE_FIREBASE_APP_ID</code> to <code className="bg-black/10 px-1 rounded">frontend/.env</code> (from Firebase Console → Project settings → Your apps).
                    </div>
                )}
                ...
                {!resetMode && (
                    <>
                        ...
                        <button
                            ...
                            className="w-full flex items-center justify-center gap-2 border border-border-default rounded py-sm text-sm font-medium text-text-primary hover:bg-surface-hover transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
